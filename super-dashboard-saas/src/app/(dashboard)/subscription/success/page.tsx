    ...
    if (isLoadingKey) {
      return (
        <div className="my-4 flex flex-col items-center">
          <LoadingSpinner />
          <p className="text-gray-600 mt-2 text-sm">Loading installation code...</p>
        </div>
      );
    }

    if (errorApiKey) {
      return <p className="text-red-500 bg-red-100 p-3 rounded mt-4">Error loading installation code: {errorApiKey}</p>;
    }
...
<body>
  ...
  <script>
    ...
    window.setTimeout(initializeWidget, 300);
      ...
      return (
        <div className="mt-6 border-t pt-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-3">Get Started: Install Your Widget</h2>
          ...
          <p className="mt-4 text-xs text-gray-500">
            ...
            <Link href="/installation" legacyBehavior>
              <a className="text-indigo-600 hover:underline cursor-pointer">Installation Guide</a>
            </Link>.
          </p>
        </div>
      );
    ...
    return <p className="text-gray-500 mt-4 text-sm">Could not retrieve installation code.</p>;
  }; // End of renderInstallationCode function definition


  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center p-6 bg-gray-50">
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-lg w-full">
        ...
        <h1 className="text-2xl font-semibold text-gray-800 mb-3">Payment Successful!</h1>

        {/* Subscription Verification Status */}
        {isSubscriptionLoading && (
          <div className="my-4 flex flex-col items-center">
            <LoadingSpinner />
            <p className="text-gray-600 mt-2 text-sm">Verifying your subscription status...</p>
          </div>
        )}
        {subscriptionError && !isSubscriptionLoading && (
           <p className="text-red-600 my-4 text-sm">Error verifying subscription: {subscriptionError}. Please check your dashboard or contact support.</p>
        )}
        {!isSubscriptionLoading && !subscriptionError && hasActiveSubscription && (
           <p className="text-green-700 font-medium mb-4">Your subscription is now active. Thank you!</p>
        )}
        {!isSubscriptionLoading && !subscriptionError && !hasActiveSubscription && (
           <p className="text-orange-600 my-4 text-sm">Verifying subscription... If activation takes longer than expected, please check back shortly or contact support.</p>
        )}
        ...
        <div className="mt-8 space-y-3 border-t pt-6">
          <Link href="/dashboard">
            <span className="block w-full text-center px-6 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
              Go to Dashboard
            </span>
          </Link>
